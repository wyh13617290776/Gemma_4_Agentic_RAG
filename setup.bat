...
set REQ_GEMMA=requirements.txt
set /p USER_REQ_G="1. 请输入 Gemma 4 依赖路径 (默认: ./requirements.txt): "
if not "!USER_REQ_G!"=="" set REQ_GEMMA=!USER_REQ_G!

set REQ_MINERU=tools/mineru_env/requirements.txt
set /p USER_REQ_M="2. 请输入 MinerU 依赖路径 (默认: ./tools/mineru_env/requirements.txt): "
...
for %%I in ("!REQ_GEMMA!") do set "DIR_G=%%~dpI"
if exist "!DIR_G!torch*.whl" (
    echo 探测到本地包，正在安装 Gemma 专用 Torch...
    for %%f in ("!DIR_G!torch*.whl" "!DIR_G!torchvision*.whl") do pip install "%%f" -i !PYPI_MIRROR!
)
...
for %%I in ("!REQ_MINERU!") do set "DIR_M=%%~dpI"
if exist "!DIR_M!torch*.whl" (
    echo 探测到本地包，正在安装 MinerU 专用 Torch...
    for %%f in ("!DIR_M!torch*.whl" "!DIR_M!torchvision*.whl") do pip install "%%f" -i !PYPI_MIRROR!
)
